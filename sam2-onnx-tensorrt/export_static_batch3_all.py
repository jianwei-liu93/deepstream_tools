...
def export_image_encoder(model,onnx_path, batch_size=3):
    ...
    input_img = torch.randn(batch_size, 3,1024, 1024).cpu()
    out = model(input_img)
...
def export_memory_attention(model,onnx_path, batch_size=3):
    ...
    current_vision_feat = torch.randn(batch_size,256,64,64)      #[batch_size, 256, 64, 64]
    current_vision_pos_embed = torch.randn(4096,batch_size,256)  #[4096, batch_size, 256]
...
def export_mask_decoder(model,onnx_path, batch_size=3):
    ...
    high_res_feats_0 = torch.randn(batch_size,32,256,256).cpu()
    high_res_feats_1 = torch.randn(batch_size,64,128,128).cpu()
...
def export_memory_encoder(model,onnx_path, batch_size=3):
    ...
    pix_feat = torch.randn(batch_size,256,64,64)
