...
def export_image_encoder(model,onnx_path):
    ...
    input_img = torch.randn(batch_size, 3, 640, 640, dtype=torch.float32).cpu()
    out = model(input_img)
...
def export_memory_attention(model,onnx_path):
    ...
    current_vision_feat = torch.randn(batch_size, 256, 40, 40, dtype=torch.float32).cpu()
    current_vision_pos_embed = torch.randn(1600, batch_size, 256, dtype=torch.float32).cpu()
    memory_0 = torch.randn(batch_size, num_obj_ptr, 256, dtype=torch.float32).cpu()
    memory_1 = torch.randn(batch_size, num_mask, 64, 40, 40, dtype=torch.float32).cpu()
    memory_pos_embed = torch.randn(batch_size, num_mask*1600+4*num_obj_ptr, 64, dtype=torch.float32).cpu()      #[1,y*1600,64]
...
def export_mask_decoder(model,onnx_path):
    ...
    test_image_embed = torch.randn(batch_size, 256, 40, 40, dtype=torch.float32).cpu()
    test_high_res_feats_0 = torch.randn(batch_size, 32, 160, 160, dtype=torch.float32).cpu()
    test_high_res_feats_1 = torch.randn(batch_size, 64, 80, 80, dtype=torch.float32).cpu()

    out = model(
        point_coords = point_coords,
        point_labels = point_labels,
        # self.image_size = 640frame_size,
...
def export_memory_encoder(model,onnx_path):
    ...
    batch_size = 1
    test_mask_for_mem = torch.randn(batch_size, 1, 640, 640, dtype=torch.float32).cpu()
    test_pix_feat = torch.randn(batch_size, 256, 40, 40, dtype=torch.float32).cpu()
